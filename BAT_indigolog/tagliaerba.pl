...
/* REACTIVE CONTROLLER SECTION*/
proc(control(reactive_cut), [prioritized_interrupts(
        [interrupt()]),
         say("Waiting at charging station, thanks...")]).
...
poss(move,(batteryLevel > 10)).

/* cutting action */
prim_action(cutGrass(C1)) :- cell(C1).
poss(cutGrass(C1),(robotAt(C1),hasGrass(C1))).
poss(cutGrass,(batteryLevel > 10)).
...
/* SUCCESSOR STATE AXIOMS */ 
causes_false(cutGrass,hasGrass,true).
causes_val(cutGrass,batteryLevel,N,N is batteryLevel-2).
